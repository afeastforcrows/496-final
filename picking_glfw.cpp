...
public:
  ...
  bool render( ){
	...
	if (isKeyPressed(GLFW_KEY_LEFT_CONTROL) && mouseButtonFlags( ) == GLFWApp::MOUSE_BUTTON_LEFT){
		...
		startPoint = endPoint;
		endPoint = Vec3( (mousePosition[0]-250)/(250) , 0, -(mousePosition[1]-250)/(250) ) + eyePosition;

		Vec3 startVec = centerPosition-startPoint;
		Vec3 endVec = centerPosition-endPoint;
